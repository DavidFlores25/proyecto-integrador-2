...
def registrar_reserva_cliente():
    ...
    reservas_del_dia = db.obtener_reservas_por_dia(dia)  # Obtener reservas del día desde la base de datos
    
    for m in mesas_disponibles:
        ocupado = False
        for res in reservas_del_dia:
    ...
    db.crear_reserva(nombre, num_personas, dia, hora_inicio_str, hora_fin_str, inicio_min, fin_min, mesas)
...
def ver_estado_reserva():
    ...
    resultados = db.obtener_reservas_por_nombre(nombre_reserva)  # Obtener reservas por nombre desde la base de datos
...
def login_vendedor():
    ...
    contrasena_guardada = db.obtener_contrasena_vendedor(nombre)
...
def gestionar_reserva_vendedor():
    ...
    while True:
        reservas_pendientes = db.obtener_reservas_pendientes()  # se vuelve a pedir para que esté actualizada
        if not reservas_pendientes:
            print("No hay reservas pendientes.")
            break
        ...
        try:
            ...
            if 1 <= seleccion <= len(reservas_pendientes):
                ...
                reservas_aceptadas = db.obtener_reservas_aceptadas()
        ...
        if opcion == "0":
            ...
            if db.existe_vendedor(nombre):
               ...
               if db.existe_vendedor(nombre):
                 ...
                 db.actualizar_contrasena_vendedor(nombre, contrasena)
